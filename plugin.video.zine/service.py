...
def checkVersion(xs='Zine'):
    try:
        ...
        if xs.lower() == 'xship':
            ...
            addonId = 'plugin.video.Zine'
            if not xbmc.getCondVisibility("System.HasAddon(%s)" % addonId):
                xbmc.executebuiltin('InstallAddon(%s)' % addonId)
                xbmc.executebuiltin('SendClick(11)')
            try: addonInfo = cConfig(addonId).getAddonInfo
            except: return
            url = 'https://raw.githubusercontent.com/streamZine/ZineRepo/refs/heads/repo/zips/plugin.video.Zine/addon.xml'
            url2 = 'https://github.com/streamZine/ZineRepo/raw/refs/heads/repo/zips/plugin.video.Zine/%s'
        else: return
        ...
        # if addonVersionInt > remoteVersionInt + 100 and xs == 'Zine':
...
def main():
    ...
    if cConfig().getAddonInfo('id') == 'plugin.video.Zine':
        checkVersion('Zine')

    if cConfig().getSetting('githubUpdateDevZine') == 'true':
        status1 = updateManager.ZineDevUpdate(True)
        cRequestHandler('').clearCache()  # Cache löschen
        if cConfig().getSetting('update.notification') == 'full':  # Benachrichtung Zine vollständig
    ...
    checkDependence('plugin.video.Zine')
