...
def parseUrl():
    ...
    if params.exist('function'):
        ...
        try:
            ...
            if sLink:
                ...
                log(cConfig().getLocalizedString(30166) + ' -> [Zine]: Could not play remote url %s ' % sLink, LOGNOTICE)
        except resolver.resolver.ResolverError as e:
            log(cConfig().getLocalizedString(30166) + ' -> [Zine]: ResolverError: %s' % e, LOGERROR)
        return
    else:
        sFunction = 'load'
    ...
    log(cConfig().getLocalizedString(30166) + " -> [Zine]: Call function '%s' from '%s'" % (sFunction, sSiteName), LOGNOTICE)
...
def showMainMenu(sFunction):
    ...
    if not aPlugins:
        log(cConfig().getLocalizedString(30166) + ' -> [Zine]: No activated Plugins found', LOGNOTICE)
...
def vodGuiElements(sFunction): # Vod Menü
    ...
    if not aPlugins:
        log(cConfig().getLocalizedString(30166) + ' -> [Zine]: No activated Vod Plugins found', LOGNOTICE)
...
def settingsGuiElements():
    ...
    # GUI Zine Einstellungen
    oGuiElement = cGuiElement()
    oGuiElement.setTitle(cConfig().getLocalizedString(30042))
    oGuiElement.setSiteName('Zine')
    oGuiElement.setFunction('display_settings')
    oGuiElement.setThumbnail(os.path.join(ART, 'Zine_settings.png'))
    ZineSettings = oGuiElement
    ...
    return PluginInfo, ZineSettings, resolveurlSettings, DevUpdateMan
...
def searchGlobal(sSearchText=False):
	...
	def worker(pluginEntry):
		log(cConfig().getLocalizedString(30166) + ' -> [Zine]: Searching for %s at %s' % (sSearchText, pluginEntry['id']),LOGNOTICE)
...
def searchAlter(params):
    ...
    def worker(pluginEntry):
        log(cConfig().getLocalizedString(30166) + ' -> [Zine]: Searching for ' + searchTitle + pluginEntry['id'], LOGNOTICE)
    ...
    for result in oGui.searchResults:
        ...
        log(cConfig().getLocalizedString(30166) + ' -> [Zine]: Site: %s Titel: %s' % (guiElement.getSiteName(), guiElement.getTitle()), LOGNOTICE)
