        ...
        def onClick(self, controlId):
            if controlId == 11:
                ...
                try:
                    ...
                    return
                self.setProperty('Zine_menu', 'Person')
            elif controlId == 9:
                ...
                item = self.getControl(controlId).getSelectedItem()
                self.close()
                xbmc.executebuiltin("Container.Update(%s?function=searchTMDB&%s)" % ('plugin://plugin.video.Zine/', urlencode({'searchTitle': item.getLabel()})))
                return
