...
class cPluginHandler:
    ...
    def __updateSettings(self, pluginDB):
        ...
        try:
            ...
            # Hauptsektion für Zine Plugin finden
            Zine_section = None
            for section in root.findall('section'):
                if section.get('id') == 'plugin.video.Zine':
                    Zine_section = section
                    break
            ...
            # Kategorien finden
            for category in Zine_section.findall('category'):
    ...
    def pluginInfo(self):
        ...
        list_of_plugins = []
        fileNames = self.__getFileNamesFromFolder(self.defaultFolder) # Hole Plugins aus Zine
        for fileName in fileNames:
        ...
        Dialog().textviewer(cConfig().getLocalizedString(30265),
            ...
            + cConfig('repository.Zine').getAddonInfo('name') + ':  ' + cConfig('repository.Zine').getAddonInfo('id') + ' - ' + cConfig('repository.Zine').getAddonInfo('version') + '\n'  # Zine Repository ID und Version
            + RESOLVEURL
            + '\n'  # Absatz
