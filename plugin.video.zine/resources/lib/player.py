...
class ZinePlayer(xbmc.Player):
    ...
    def onPlayBackStopped(self):
        ...
        if self.from_global_search:
            try:
                xbmc.executebuiltin('Container.Update(plugin://plugin.video.Zine/)')
                log('Zine -> [player]: Returning to addon main menu after Global Search', LOGNOTICE)
            except Exception as e:
                log('Zine -> [player]: Failed to return to main menu: %s' % str(e), LOGERROR)
