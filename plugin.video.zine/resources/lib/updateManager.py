...
# Zine Dev
def ZineDevUpdate(silent=False):
    username = cConfig().getSettingString('Zine.dev.username')
    plugin_id = cConfig().getSettingString('Zine.dev.id')
    branch = cConfig().getSettingString('Zine.dev.branch')
    token = cConfig().getSettingString('Zine.dev.token')
    try:
...
def devUpdates():  # für manuelles Updates vorgesehen
    try:
        ...
        #pluginupdate = False # Zine Update
        # Einleitungstext
        ...
        #elif result == 1:  # Zine aktualisieren
