...
#kasi
    ...
    def __createContextMenu(self, oGuiElement, listitem, bIsFolder, sUrl):
        ...
        if oGuiElement._mediaType == 'movie' or oGuiElement._mediaType == 'tvshow':
            if cConfig().getSetting('Zine.trailer') == 'true':
                if not xbmc.getCondVisibility('System.HasAddon(%s)' % 'script.module.Zine.trailer'):  # Schauen ob Addon installiert
                    xbmc.executebuiltin('InstallAddon(%s)' % 'script.module.Zine.trailer')  # Addon installieren
                contextitem.setTitle(cConfig().getLocalizedString(30027))  # Trailer Funktion
                contextmenus += [(contextitem.getTitle(), "RunPlugin(plugin://script.module.Zine.trailer/?action=play&name=%s&url=&language=)" % (itemValues['title'],),)]
        if oGuiElement._mediaType == 'movie' or oGuiElement._mediaType == 'tvshow':
    ...
    def setView(self, content='movies'):
        ...
        # sets view to the viewID which is selected in Zine settings
    ...
    @staticmethod
    def showInfo(sTitle='Zine', sDescription=cConfig().getLocalizedString(30253), iSeconds=0):
        if iSeconds == 0:
            iSeconds = 1000
        else:
            iSeconds = iSeconds * 1000
    ...
    @staticmethod
    def showLanguage(sTitle='Zine', sDescription=cConfig().getLocalizedString(30403), iSeconds=0):
        if iSeconds == 0:
            iSeconds = 1000
        else:
            iSeconds = iSeconds * 1000
