...
class cHosterGui:
        ...
        try:
            ...
            if siteResult:
                ...
                if mediaUrl:
                    ...
                    oGui.showError('Zine', cConfig().getLocalizedString(30134), 5)
                    return False
            elif mediaUrl:
                logger.info('-> [hoster]: resolve: ' + mediaUrl)
            ...
            oGui.showError('Zine', cConfig().getLocalizedString(30135), 7)
            return False
        # resolver response
    ...
    def sendToMyJDownloader(self, sMediaUrl=False, sMovieTitle='Zine'):
    ...
    def stream(self, playMode, siteName, function, url):
        ...
        self.dialog.create('Zine', cConfig().getLocalizedString(30138))
        ...
        if not siteResult:
            self.dialog.close()
            cGui().showInfo('Zine', cConfig().getLocalizedString(30141))
            return
        ...
        if 'name' in siteResult[0]:
            ...
            if not siteResult:
                self.dialog.close()
                cGui().showInfo('Zine', cConfig().getLocalizedString(30142))
                return
            ...
            if not siteResult:
                self.dialog.close()
                cGui().showInfo('Zine', cConfig().getLocalizedString(30144))
                return False
        ...
        self.dialog = xbmcgui.DialogProgress()
        self.dialog.create('Zine', cConfig().getLocalizedString(30145))
    ...
    def streamAuto(self, playMode, siteName, function):
        ...
        if 'name' in siteResult[0]:
            ...
            if len(siteResult) > self.maxHoster:
                siteResult = siteResult[:self.maxHoster - 1]
            check = False
            self.dialog.create('Zine', cConfig().getLocalizedString(30147))
            total = len(hosters)
